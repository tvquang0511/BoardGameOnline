...
exports.list = async (req, res, next) => {
  try {
    ...
    const friends = await Promise.all(
      rows.map(async (rel) => {
        const friendId = rel.user_low_id === req.user.sub ? rel.user_high_id : rel.user_low_id;
...
exports.requests = async (req, res, next) => {
  try {
    ...
    const requests = await Promise.all(
      rows.map(async (rel) => {
        const requesterProfile = await getFriendProfile(rel.requester_id, req.user.sub);
...
exports.outgoing = async (req, res, next) => {
  try {
    ...
    const requests = await Promise.all(
      rows.map(async (rel) => {
        const addresseeProfile = await getFriendProfile(rel.addressee_id, req.user.sub);
      ...
      switch (error.message) {
        ...
        case 'REQUEST_ALREADY_RECEIVED':
          return res.status(400).json({ 
            message: 'Người này đã gửi lời mời cho bạn. Vui lòng kiểm tra mục "Lời mời kết bạn" để chấp nhận' 
          });
        case 'ALREADY_FRIENDS':
          return res.status(400).json({ message: 'Bạn đã là bạn bè với người này rồi' });
        default:
          throw error;
      }
    }
...
exports.unfriend = async (req, res, next) => {
  try {
    ...
    if (!row) return res.status(404).json({ message: 'Không tìm thấy mối quan hệ bạn bè' });
    res.json({ ok: true });
  } catch (e) {
    next(e);
  }
